...
def fill_array_random(arr, max_value=None):
    if max_value is None:
        max_value = (len(arr))-1

    for i in range(len(arr)):
        arr[i] = urandom.randint(0, max_value)

def benchmark_sort_array(arr, sort_function, sort_name, *args):
    start_time = utime.ticks_us()
    if args:
        ...
        sort_function(arr)
    duration = utime.ticks_diff(utime.ticks_us(), start_time)
    result = {"name": "sortArray_" + sort_name, "duration": duration}
    print(json.dumps(result))

def benchmark_digital_read():
    def operation():
        machine.Pin(PIN_TO_TEST, machine.Pin.IN).value()
    benchmark_operation(operation, "digitalRead")

def benchmark_digital_write():
    def operation():
        pin = machine.Pin(PIN_TO_TEST, machine.Pin.OUT)
        pin.on()
        pin.off()
    benchmark_operation(operation, "digitalWrite")

def benchmark_pin_mode():
    def operation():
        machine.Pin(PIN_TO_TEST, machine.Pin.OUT)
    benchmark_operation(operation, "pinMode")

def benchmark_multiply_byte():
    def operation():
        a = 10
        b = 5
        result = a * b
    benchmark_operation(operation, "multiply byte")

def benchmark_divide_byte():
    def operation():
        a = 10
        b = 5
        result = a // b
    benchmark_operation(operation, "divide byte")

def benchmark_add_byte():
    def operation():
        a = 10
        b = 5
        result = a + b
    benchmark_operation(operation, "add byte")

def benchmark_multiply_integer():
    def operation():
        a = 1000
        b = 5
        result = a * b
    benchmark_operation(operation, "multiply integer")

def benchmark_divide_integer():
    def operation():
        a = 1000
        b = 5
        result = a // b
    benchmark_operation(operation, "divide integer")

def benchmark_add_integer():
    def operation():
        a = 1000
        b = 5
        result = a + b
    benchmark_operation(operation, "add integer")

def benchmark_multiply_long():
    def operation():
        a = 1000000
        b = 5
        result = a * b
    benchmark_operation(operation, "multiply long")

def benchmark_divide_long():
    def operation():
        a = 1000000
        b = 5
        result = a // b
    benchmark_operation(operation, "divide long")

def benchmark_add_long():
    def operation():
        a = 1000000
        b = 5
        result = a + b
    benchmark_operation(operation, "add long")

def benchmark_multiply_float():
    def operation():
        a = 3.14
        b = 2.0
        result = a * b
    benchmark_operation(operation, "multiply float")

def benchmark_divide_float():
    def operation():
        a = 3.14
        b = 2.0
        result = a / b
    benchmark_operation(operation, "divide float")

def benchmark_add_float():
    def operation():
        a = 3.14
        b = 2.0
        result = a + b
    benchmark_operation(operation, "add float")

def benchmark_itoa():
    def operation():
        num = 12345
        buf = str(num)
    benchmark_operation(operation, "itoa()")

def benchmark_ltoa():
    def operation():
        num = 1234567890
        buf = str(num)
    benchmark_operation(operation, "ltoa()")

def benchmark_dtostrf():
    def operation():
        num = 123.456
        buf = "{:.3f}".format(num)
    benchmark_operation(operation, "dtostrf()")

def benchmark_random():
    def operation():
        urandom.randint(0, 100)
    benchmark_operation(operation, "random()")

def benchmark_bit_operations():
    def operation():
        x = 3
        y = 0
        y |= (1 << x)
    benchmark_operation(operation, "y |= (1<<x)")

def benchmark_analog_read():
    def operation():
        adc = machine.ADC(PIN_TO_TEST)
        adc.read()
    benchmark_operation(operation, "analogRead")

def benchmark_pwm_write():
    def operation():
        pwm = machine.PWM(machine.Pin(PIN_TO_TEST))
        pwm.duty(512)  # PWM-Duty Cycle setzen
    benchmark_operation(operation, "digitalWrite() PWM")

def benchmark_delay():
    def operation():
        utime.sleep_ms(1)
    benchmark_operation(operation, "delay(1)")

def benchmark_delay_100():
    def operation():
        utime.sleep_ms(100)
    benchmark_operation(operation, "delay(100)")

def benchmark_delay_microseconds_2():
    def operation():
        utime.sleep_us(2)
    benchmark_operation(operation, "delayMicroseconds(2)")

def benchmark_delay_microseconds_5():
    def operation():
        utime.sleep_us(5)
    benchmark_operation(operation, "delayMicroseconds(5)")

def benchmark_delay_microseconds_100():
    def operation():
        utime.sleep_us(100)
    benchmark_operation(operation, "delayMicroseconds(100)")
...
def setup():
    ...
    benchmark_sort_array(test_array, counting_sort, "counting", max_value)
